...
export function getDestinationCategoryById(
  id: string,
...
export function getVenueGalleryImages(venueId: string): Venue["images"] {
  return (
    getVenueById(venueId)?.images.filter((img) => img.type === "GALLERY") ?? []
  );
}
...
export function getWeddingExperienceById(
  id: string,
): WeddingExperience | undefined {
  return weddingExperienceList.find((e) => e.id === id);
}

export function getWeddingExperienceByCategory(
  category: string,
): WeddingExperience | undefined {
...
export function getWeddingThemesByType(
...
export function getWeddingThemesByExperience(
  experienceId: string,
): HydratedTheme[] {
...
export function getThemesByDestination(
  ...
  const venueIds = new Set(
    getVenuesByDestination(destinationId).map((v) => v.id),
  );
  return weddingThemeList
