...
// ─── CATEGORY_META ─────────────────────────────────────────────────────────────
// Tetap dipertahankan persis: ini pure UI config, tidak perlu dari DB.
// label, anchor, description, dan locations[] untuk filter adalah keputusan
// editorial/design — bukan data dinamis.
...
> = {
  "cat-bali": {
    ...
    locations: [
      ...
      { label: "Ubud & Gianyar", value: "Ubud & Gianyar" },
      { label: "East Bali", value: "East Bali" },
      { label: "North Bali", value: "North Bali" },
      { label: "West Bali", value: "West Bali" },
      { label: "Highlands & Mountains", value: "Highlands, Lakes and Mountains" },
    ],
  },
  "cat-themes": {
    label: "Themes",
    ...
    locations: [
      ...
      { label: "Waterfall Weddings", value: "Ubud & Gianyar, North Bali, West Bali" },
      { label: "Private Villa", value: "South Bali, Ubud & Gianyar, East Bali, North Bali, West Bali" },
      { label: "Mountain Weddings", value: "Highlands, Lakes and Mountains" },
      { label: "Jungle / Forest", value: "Ubud & Gianyar, East Bali" },
      { label: "Beachfront / Oceanfront", value: "South Bali, East Bali, North Bali" },
      { label: "Royal Balinese", value: "Ubud & Gianyar, East Bali" },
      { label: "Rice Paddy Field", value: "Ubud & Gianyar, East Bali, West Bali" },
      { label: "Riverside", value: "Ubud & Gianyar, East Bali, West Bali" },
      { label: "Garden Weddings", value: "South Bali, Ubud & Gianyar, Highlands, Lakes and Mountains" },
      { label: "Chapel Weddings", value: "South Bali" },
    ],
  },
  "cat-islands": {
    label: "Islands",
...
interface CategorySectionProps {
  ...
  // UUID dari DB yang sudah di-resolve oleh useDestinations hook
  dbCategoryId: string | undefined;
...
function CategorySection({
  ...
  // Filter: cocokkan category_id DB (uuid) dan location string
  const allDestinations = destinations.filter(
    (d) =>
      // Jika DB uuid sudah diketahui, filter by uuid; jika belum (loading), tampilkan kosong
      dbCategoryId !== undefined &&
      d.category_id === dbCategoryId &&
      matchesLocation(d, selectedValue),
  );
...
export default function DestinationsPage() {
  ...
  const { destinations, categoryIdMap, isLoading, error } = useDestinations();

  // ── Scroll tracking untuk active anchor ──────────────────────────────────
  useEffect(() => {
    ...
    <main className="relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 sm:px-8 md:px-16 lg:px-24">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {(
              ...
              // Hitung count dari data DB (via uuid mapping)
              const dbId = categoryIdMap[id];
              const count = dbId
                ? destinations.filter((d) => d.category_id === dbId).length
                : 0;
      ...
      {(Object.keys(CATEGORY_META) as CategoryId[]).map((categoryId) => (
        <CategorySection
          key={categoryId}
          categoryId={categoryId}
          dbCategoryId={categoryIdMap[categoryId]}
