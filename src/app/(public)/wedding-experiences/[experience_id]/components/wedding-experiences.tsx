      ...
      <div className="grid lg:grid-cols-12 gap-12 lg:gap-20 items-start">
        <div className="lg:col-span-5 space-y-8">
          ...
          <motion.div variants={fadeInUp} className="space-y-0">
            {data.intro.listLabel && (
              <p className="text-primary italic mb-4">{data.intro.listLabel}</p>
            )}
            {data.intro.list.map((item, i) => (
              <div
                key={item}
      ...
      <div className="container mx-auto px-4 sm:px-8 md:px-16 lg:px-24">
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          <div className="lg:col-span-6 space-y-8">
            ...
            <motion.div variants={fadeInUp}>
              {data.approach.listLabel && (
                <p className="text-primary italic mb-5">
                  {data.approach.listLabel}
                </p>
              )}
      ...
      <div className="container mx-auto px-4 sm:px-8 md:px-16 lg:px-24">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16">
          ...
          <motion.div variants={fadeInUp} className="space-y-4">
            {data.services.list.map((item, i) => (
              <div
                key={item}
                className="flex items-start gap-4 pb-4 border-b border-primary/20 last:border-0"
              >
                <span className="text-primary font-mono w-5">
                  {String(i + 1).padStart(2, "0")}
                </span>
                <span className="text-primary">{item}</span>
              </div>
            ))}
            <p className="text-primary italic pt-2">{data.services.footnote}</p>
            <div className="pt-4">
              <Link href="https://wa.me/628113980998" target="_blank">
            ...
            <div className="bg-primary p-8 lg:p-10 h-full flex flex-col justify-between">
              <div>
                <p className="text-white tracking-[0.25em] uppercase mb-3">
                  {data.services.darkPanel.label}
                </p>
                <h3 className="text-2xl text-white font-semibold mb-4">
                  {data.services.darkPanel.heading[0]}
                  <br />
                  <span className="italic font-light">
                    {data.services.darkPanel.heading[1]}
                  </span>
                </h3>
                <p className="text-white leading-relaxed mb-8">
                  {data.services.darkPanel.body}
                </p>
                <div className="space-y-4">
                  {data.services.darkPanel.list.map((item, i) => (
                    <div key={item} className="flex items-center gap-4">
                      <span className="text-white font-mono w-5">
                        {String(i + 1).padStart(2, "0")}
                      </span>
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-8 md:px-16 lg:px-24">
        {data.closing.coupleValues.length > 0 ? (
          ...
          <div className="grid lg:grid-cols-12 gap-12">
            ...
            {data.closing.coupleLabel && (
              <div className="lg:col-span-6 space-y-1">
                <motion.div variants={fadeInUp}>
                  <p className="text-white italic mb-5">
                    {data.closing.coupleLabel}
                  </p>
                  {data.closing.coupleValues.map((v) => (
                    <div
                      key={v}
...
// Mapping from experience slug to venue filter category
const slugToVenueCategory: Record<
  string,
  "luxury" | "private_villa" | "elopement" | "intimate"
> = {
  "luxury-weddings": "luxury",
  "private-villa-weddings": "private_villa",
...
function SectionVenueList({ experienceSlug }: { experienceSlug: string }) {
  const category = slugToVenueCategory[experienceSlug];

  // Only show this section for slugs that have a matching venue category
  ...
  const filteredVenues = useMemo(() => {
    ...
    } else if (category === "private_villa") {
      list = list.filter(
        (v) => v.categoryRelations?.category === "private_villa",
      );
    } else if (category === "elopement") {
...
const subExperiences = [
  ...
  //   subtitle: "in Bali",
  //   tag: "Destination",
  //   desc: "A destination wedding in Bali shaped by atmosphere, culture, landscape, and emotion — intentional, refined, and deeply personal.",
...
function SectionSubExperiences({ currentSlug }: { currentSlug: string }) {
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 sm:px-8 md:px-16 lg:px-24">
        ...
        {/* Grid — 2 kolom, item aktif disembunyikan */}
              ...
              <Link href={exp.href} className="group block">
                ...
                <div>
                  <h3 className="text-primary font-semibold text-xl group-hover:text-primary/70 transition-colors">
                    {exp.title}{" "}
                    {exp.subtitle && (
                      <span className="italic font-light">{exp.subtitle}</span>
                    )}
                  </h3>
...
export function WeddingExperiences({
  experience_id,
}: {
  experience_id: string;
}) {
  const data = slugData[experience_id as SlugKey];
  ...
  const heroMap = {
    split: <HeroSplit data={data} />,
    bottom: <HeroBottomSplit data={data} />,
    centered: <HeroCentered data={data} />,
    editorial: <HeroEditorial data={data} />,
  };

  return (
    <main className="relative overflow-hidden">
      {heroMap[data.hero.style]}
      <SectionIntro data={data} />
      <SectionApproach data={data} />
